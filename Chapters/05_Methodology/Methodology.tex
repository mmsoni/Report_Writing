...
Once the channel flow becomes turbulent the flow is allowed to further develop in time until the statistically steady state is reached i.e. the statistics of the flow variables are invariant to temporal changes. Once the stat. steady state is reached the time averaging of the flow variables is carried out and these time averaged quantities are further averaged over in space along the two homogeneous directions i.e spatial averaging over the x-z planes. \\

To determine the statistically steady state in the channel one can observer the evolution of the several quantities viz. (volume averaged)turbulent kinetic energy, $\tau_w$, $u_\tau$ etc. over some non-dimensional time. Refer to ~\cite{Jimandmoin:91} for the detailed description for steady state determination and spatial averaging. Here two non-dimensional time will be specified \emph{large-eddy turn-over-time (LETOT)}, $tu_\tau/\de$, and the flow through the domain time, $tU_b/\de$. LETOT is the time required by the large eddy to breakdown and transfer the energy to the small eddies~\cite{bespalko:11}. In this investigation no specific quantity had been monitored to determine the statistically steady state. An estimation that $\sim10LETOT$ is sufficient for the flow to be fully developed and statistically steady~\cite{mk}.  \\

Since LB-LES simulations are performed. In LB-LES a non-dimensional parameter mach number, \emph{\textbf{Ma}}, is very important. Since the LBM is only applicable to the incompressible flows \emph{\textbf{Ma}}$\leq 0.3$ is the upper-limit. Since the spatial resolution is fixed the temporal resolution can only be fixed by varying the \emph{\textbf{Ma}} number. In our case with the given physical parameters the \emph{\textbf{Ma}} number was very small and it has been artificially increased to increase the time-step size. The respective settings in lb units is specified in the Appendix.  
The following table shows the details of the simulation settings:
%
\begin{table}[h!]
\begin{center}
...
\hline
 &$\sim time$ & Mesh1 & Mesh1\_5 & Mesh2 \\
  \hline
  \multirow{3}{6em}{Flow developing stage} & $t\ [s]$ & 3 & 5 & 8\\
  & $tu_\tau/\delta$ & 300 & 1 & 2\\ 
  & $tU_b/\delta$ & 100 & 1 & 2\\ 
  \hline
  \multirow{3}{6em}{Averaging stage} & $t [s]$ & 3 & 5 & 8\\
  & $tu_\tau/\delta$ & 300 & 1 & 2\\ 
  & $tU_b/\delta$ & 100 & 1 & 2\\ 
  \hline
\end{tabular}
